...
const Robot: React.FC<RobotProps> = (props) => {
  // Auto-rotation corners for the robot, useMemo used to compute once during mount only and store.
  ...
  //refs which represent the container(group) and the actual model.
  ...
  //   useGLTF used to load glb and gltf type models.
  const { scene, animations } = useGLTF(
    `${import.meta.env.BASE_URL}roboModels/r1.glb`
  );

  //   useAnimations renders the inbuilt animations within the model file.
  ...
  // Play inbuilt animation -- halo effect under bot
  useEffect(() => {
    if (!actions) return;
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (e: PointerEvent) => {
      // Update mouse position & convert it to normalized coordinates for 3js
  ...
  useFrame(() => {
    ...
    const resetDelay = 2000; // delay after which to return to normal, after no user input
    const resetSpeed = 0.05; // speed at which to return to normal, after no user input
